...
**Calibration result** (`judge/calibration_set.jsonl`, n=10, **author-labeled — not yet independently human-reviewed**, see Open Questions): **60% exact-label agreement overall (6/10)**. Corrected breakdown (an earlier draft of this report miscounted this split — verified by direct recomputation, see Audit):
- On the **8** items labeled `PASS`/`FAIL`: **6/8 (75%) agreement**. Both disagreements (`cal-003`, `cal-008`) were the judge hedging a `PASS`-labeled item to `BORDERLINE` — **zero instances of the judge flipping polarity** (never called `FAIL` on a `PASS` item or vice versa).
- On the **2** items labeled `BORDERLINE` (`cal-004`, `cal-009`): **0/2 agreement** — the judge resolved both to a firm `FAIL`/`PASS` instead.

**Interpretation**: the judge never made a dangerous polarity error (no false-PASS on a real FAIL, no false-FAIL on a real PASS) across all 10 items, which is the property EC-002's threshold actually depends on — but its exact-match reliability on the binary label is 75%, not 100%, and its `BORDERLINE` calls are unreliable on this sample (0/2). **Recommendation carried into EC-002's threshold**: use the judge's `PASS`/`FAIL` calls as the real gate, since it never flipped polarity here; treat any `BORDERLINE` verdict it produces as inherently untrustworthy until a larger, independently human-labeled set is built (see Open Questions) — do not yet treat `BORDERLINE` as its own reliable middle category, and do not treat 75% binary agreement as production-ready without a larger calibration sample.
...
| Judge calibration, binary PASS/FAIL (8 items) | **6/8 (75%) agreement** — both misses were `PASS`→`BORDERLINE` hedges, zero polarity flips |
| Judge calibration, BORDERLINE (2 items) | **0/2 agreement** — not yet calibrated, see §4 |
...
- **Live verification performed**: `python evals/checks/check_escalation_logic.py` (8/8), `python evals/checks/check_pii_redaction.py` (6/6), `python evals/checks/check_live_accuracy.py` (10/10 escalation-decision pass, real Anthropic API calls, p50=17.31s/p95=18.66s), `python evals/judge/run_judge.py --calibrate` (10 items, real `claude-opus-5` calls, 60% overall / 75% on the 8 binary-expected items / 0% on the 2 BORDERLINE-expected items).
- **Correction (2026-09-05, same-day follow-up)**: this file originally miscounted the calibration split as "6/6 (100%) binary, 0/4 BORDERLINE." Recomputed directly from `run_judge.py`'s own output while preparing a detailed per-scenario report: the correct split is 8 items labeled `PASS`/`FAIL` (6/8, 75% agreement, zero polarity flips) and 2 items labeled `BORDERLINE` (0/2 agreement) — corrected in §4 and §6 above. No re-run was needed; the underlying judge calls and verdicts were unchanged, only this file's arithmetic was wrong.
