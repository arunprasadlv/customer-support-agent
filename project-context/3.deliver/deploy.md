...
## Live Deployment (2026-09-02)

Per explicit operator instruction ("deploy the app to railway"), a live Railway project was actually created this session — the first time any of this file's config has been exercised against a real Railway account. This corrects two of `*define-deploy`'s prior Open Questions/Assumptions with real observed behavior, and records one new operational finding not previously anticipated.

### What was created

- Railway project `customer-support-agent` (id `cbec5b5a-0214-44b5-8ac2-394e7fe6287a`), single `production` environment, workspace `c98e1501-7bbb-4179-b617-0568c969135a` (the operator's own — confirmed via `whoami` before creating anything, no other projects existed).
- Two services, each connected to `github.com/arunprasadlv/customer-support-agent` on branch **`main`** (per operator's answer to a direct question this session — `devops` was merged into `main` first; see Git changes below):
  - `backend` (id `7362d72f-afec-40f7-b0ef-6ecbc7e09491`), root directory `backend/`, one Volume (`backend-state`) mounted at `/app/state`, public domain `https://backend-production-6e49.up.railway.app`.
  - `frontend` (id `5ea0418c-a0c1-442c-95b0-01e535559b0c`), root directory `frontend/`, public domain `https://frontend-production-3f02.up.railway.app`.
- Variables set (names only, per this file's existing Secrets standard): backend — `INTERACTION_LOG_DB_PATH`, `TRACE_LOG_DIR`, `CORS_ALLOWED_ORIGINS` (set to the frontend's generated domain); frontend — `VITE_API_BASE_URL` (set to the backend's generated domain, before the frontend's first build, matching the two-pass sequencing this file's `*define-deploy` section already documented).
- **`ANTHROPIC_API_KEY` was deliberately NOT set** — per operator instruction this session, the operator will add it directly in the Railway dashboard. Until then, `/health` and `/` on both services work, but `/chat`/`/email` will fail (no live LLM calls possible). This is the one remaining step before the deployment is actually functional end-to-end, not just online.

### Git changes this session (not previously committed)

Everything `*define-deploy`/`*configure-cicd`/`*document-deploy` authored (`backend/Dockerfile`, `backend/railway.json`, `frontend/Dockerfile`, `frontend/nginx.conf`, `frontend/railway.json`, `docker-compose.yml`, `.env.docker.example`, `.github/workflows/ci.yml`, this file's own prior sections) had been sitting **uncommitted/untracked** in the working tree — Railway builds from GitHub, not local disk, so none of it could have deployed until pushed. Per operator answers to two direct questions this session: committed on `devops` (`ca2b8c6`), pushed, then fast-forward-merged into `main` (`main` had zero divergent commits, so this was a clean fast-forward, no merge conflicts) and pushed. Two further fixes (below) were committed and pushed the same way, keeping `devops` and `main` in sync throughout.

### Two real bugs found and fixed during this deploy (not caught by any prior review — no live Railway account existed until this session)

1. **Frontend nginx never listened on IPv6.** First deploy attempt: backend came up healthy (`GET /health` → `200`), frontend returned `502`/`connection refused` from Railway's edge proxy despite its container logs showing nginx started cleanly. Root cause (first fix, insufficient alone): the official `nginx:1.27.3-alpine` image's entrypoint auto-patches `listen 80` to also listen on `[::]:80`, but only when `/etc/nginx/conf.d/default.conf` is byte-identical to the image's packaged default — ours wasn't (it has the SPA `try_files` fallback), so the patch silently no-opped, leaving nginx IPv4-only. Fixed by adding `listen [::]:80;` explicitly to `frontend/nginx.conf`. This did not fully fix it (see #2) but is a real, independently-worth-keeping fix.
2. **Frontend nginx never read `$PORT` at all — the actual root cause.** Even after #1, `502`s continued. Investigation via `list-domains` showed the service's domain has `targetPort: null` — Railway assigns a distinct `PORT` per deployment/instance (confirmed: it does not appear in `list-variables`' output, unlike every other injected `RAILWAY_*` var, because it's generated per-instance at runtime rather than stored as a service variable) and proxies to that exact port. `backend/Dockerfile` already handles this correctly (`--port ${PORT:-8000}`, added and verified in `*define-deploy` above) — `frontend/nginx.conf` never did, hardcoded to `80` regardless of what Railway actually proxies to. **Fix**: replaced the static `frontend/nginx.conf` with `frontend/nginx.conf.template` (`listen ${PORT}; listen [::]:${PORT};`), copied to `/etc/nginx/templates/default.conf.template` so the image's built-in `envsubst`-on-templates entrypoint step resolves `$PORT` from the real container environment before nginx starts; `frontend/Dockerfile` now sets `ENV PORT=80` as a fallback default (for local `docker run`/compose, where nothing sets `PORT` — `docker-compose.yml`'s `"${FRONTEND_HOST_PORT:-5173}:80"` mapping needed no change, since the fallback keeps the container-side port at 80 exactly as before) and the `HEALTHCHECK` now reads `${PORT}` too (already shell-form, so no CMD-form change needed, unlike the backend's earlier fix).
   - **Corrects `*define-deploy`'s prior Assumption/Open Question**: "no `healthcheckPath` set — Railway falls back to a TCP check on the exposed port, which is sufficient for a static file server" assumed `EXPOSE 80` alone was enough for Railway to know the target port. **Verified false in this session** — Railway needs the container actually listening on its assigned `$PORT`, `EXPOSE` is documentation-only exactly as this file's own backend section already said elsewhere, just hadn't been applied to the frontend. Confirmed fixed: `GET /` on the live frontend domain now returns `200`, `GET /chat` (a client-side React Router path) also returns `200` (SPA fallback confirmed working), and the served JS bundle was confirmed (via `curl`+`grep`) to contain the backend's actual Railway domain, proving `VITE_API_BASE_URL` really was passed through as a build `ARG` — **this also resolves `*define-deploy`'s other flagged Open Question** ("not yet verified: whether Railway actually forwards dashboard-set variables as build ARGs" — confirmed true, live).
   - A CORS preflight (`OPTIONS /chat` with `Origin: https://frontend-production-3f02.up.railway.app`) against the live backend returned `200` with the correct `access-control-allow-origin`, confirming `CORS_ALLOWED_ORIGINS` is correctly wired end-to-end too.

### New operational finding: git-push-to-deploy did not auto-trigger

This file's `*document-deploy` section documented Railway's git-push-to-deploy as the "primary" promotion model, flagged as not yet verified against a live project. **Partially not confirmed as described**: after pushing the nginx fixes to `main` (the branch both services are attached to), neither service started a new build within ~40 seconds of the push — `list-deployments` still showed only the prior deployment. Both rebuilds in this session were instead triggered by calling `connect-service-source` again with the same repo/branch (re-attaching a service's source appears to force a fresh build even when the repo/branch value is unchanged), not by the push itself. This may simply be a webhook propagation delay longer than this session waited out, or the MCP-driven `connect-service-source` call may not install the same GitHub App webhook that the Railway dashboard's own "Deploy from GitHub repo" flow does — genuinely not determined either way in this session. **Operator action item**: confirm in the Railway dashboard (Settings → the GitHub connection) whether push-to-deploy is actually armed for both services; if not, either enable it there, or treat "re-trigger via dashboard/CLI redeploy after each push" as the real promotion step until it is.

### Sources
- Live Railway MCP tool calls this session (`whoami`, `list-projects`, `create-project`, `create-service` ×2, `update-service` ×2, `create-volume`, `set-variables` ×3, `connect-service-source` ×4, `generate-domain` ×2, `list-domains`, `list-variables` ×2, `get-service-config`, `list-deployments`, `get-logs`, `environment-status`) — this section reports what those calls actually returned, not a plan.
- `curl` against both live public domains (`/health`, `/`, `/chat`, the JS bundle, an `OPTIONS /chat` CORS preflight) — real HTTP responses, not simulated.
- This file's own `*define-deploy`/`*document-deploy` sections above, re-read before acting, to identify which of their Assumptions/Open Questions this live attempt could actually resolve.
- Operator answers to two direct questions this session: deploy branch = `main` (not `devops`); `ANTHROPIC_API_KEY` = operator sets it in the dashboard, not provided to this session.

### Assumptions
- The IPv6-listen fix (#1 above) is kept even though it wasn't sufficient alone — it's a real, correct fix for a real (if secondary, on Railway specifically) problem, not reverted.
- "Re-attaching the same source forces a rebuild" is treated as a reliable trigger mechanism based on this session's two successful observations of it; not something Railway documents as an intended API, so it's a working technique here, not a durable contract to build automation on.
...
### Audit
- **Timestamp**: 2026-09-02 (Railway's own deployment timestamps are UTC and show 2026-09-03, a few hours ahead due to timezone — same session)
- **Persona**: `@devops.eng`
- **Action**: Live deploy — not one of `*prepare-release`/`*define-deploy`/`*configure-cicd`/`*document-deploy`'s scoped commands (all four were already complete); this was the operator's explicit, separate "deploy the app to railway" instruction, executed using the config those commands had already produced.
- **Resolved runtime**: `crewai` (unrelated — governs the application's own CrewAI runtime, not this Railway hosting action)
- **Inputs read in full**: this file's `*define-deploy`/`*document-deploy` sections (for the exact env-var matrix, volume plan, and two-pass URL sequencing, which were followed as documented and not re-decided), `frontend/Dockerfile`, `frontend/nginx.conf` (both read before diagnosing the 502s)
